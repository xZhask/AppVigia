<?php
$pestanas = [
    ['ruta' => 'catalogos/usuarios', 'etiqueta' => 'Usuarios'],
    ['ruta' => 'catalogos/grados',   'etiqueta' => 'Grados PNP'],
    ['ruta' => 'catalogos/unidades', 'etiqueta' => 'Unidades PNP'],
];
?>
<div class="page-head">
  <div>
    <div class="page-title">Unidades PNP</div>
    <div class="page-desc">Comisarías, direcciones y dependencias donde presta servicio el personal</div>
  </div>
  <div class="spacer"></div>
  <a class="btn btn-primary" href="/catalogos/unidades/nuevo">
    <svg width="14" height="14" viewBox="0 0 14 14"><path d="M7 3v8M3 7h8" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>
    Nueva unidad
  </a>
</div>

<?php require __DIR__ . '/../../partials/tabs-catalogo.php'; ?>

<div class="card table-card">
  <div class="toolbar">
    <div class="search">
      <svg width="15" height="15" viewBox="0 0 15 15"><circle cx="6.5" cy="6.5" r="4.5" stroke="currentColor" stroke-width="1.4" fill="none"/><path d="m10 10 3 3" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>
      <input placeholder="Buscar por nombre o tipo…" data-filtro-tabla="#tabla-unidades">
    </div>
  </div>
  <div style="overflow-x:auto">
  <table id="tabla-unidades">
    <thead><tr><th>Nombre</th><th>Tipo</th><th>Distrito</th><th>Estado</th><th></th></tr></thead>
    <tbody>
      <?php if (empty($unidades)): ?>
        <tr><td colspan="5" style="color:var(--muted);text-align:center;padding:32px 16px">No hay unidades registradas todavía.</td></tr>
      <?php endif; ?>
      <?php foreach ($unidades as $unidad): ?>
        <tr>
          <td class="pt-name"><?= e($unidad['nombre']) ?></td>
          <td><?= e($unidad['tipo'] ?? '—') ?></td>
          <td><?= e($unidad['distrito_nombre'] ?? '—') ?></td>
          <td><span class="state"><span class="dot <?= $unidad['activo'] ? 'st-open' : 'st-closed' ?>"></span> <?= $unidad['activo'] ? 'Activa' : 'Inactiva' ?></span></td>
          <td>
            <div class="row-actions">
              <a class="ra" title="Editar" href="/catalogos/unidades/<?= (int) $unidad['id'] ?>/editar">
                <svg width="15" height="15" viewBox="0 0 15 15"><path d="M10 2.5 12.5 5 5 12.5 2 13l.5-3L10 2.5Z" stroke="currentColor" stroke-width="1.2" fill="none" stroke-linejoin="round"/></svg>
              </a>
              <form method="post" action="/catalogos/unidades/<?= (int) $unidad['id'] ?>/alternar" onsubmit="return confirm('<?= $unidad['activo'] ? 'Desactivar' : 'Activar' ?> esta unidad?')">
                <?= \App\Core\Csrf::campoOculto() ?>
                <button class="ra" type="submit" title="<?= $unidad['activo'] ? 'Desactivar' : 'Activar' ?>">
                  <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><path d="M7.5 2v5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/><path d="M4.5 4A5 5 0 1 0 10.5 4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
                </button>
              </form>
            </div>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
  </div>
</div>
